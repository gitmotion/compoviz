...
3. Start with the production build:
...
# Access the app at http://localhost:8080 or based on your port configuration
```
...
wget https://raw.githubusercontent.com/adavesik/compoviz/refs/heads/main/docker-compose.yml
...
# Remove the image
docker image rm compoviz-local:latest
...
| Build & Start container with logging               | `npm run docker:up`                                                 | `docker compose -f docker-compose.local.yml up`                          |
| Restart running container                          | `npm run docker:restart`                                            | `docker compose -f docker-compose.local.yml restart`                     |
| Build & Start container in detached mode           | `npm run docker:up -- -d`                                           | `docker compose -f docker-compose.local.yml up -d`                       |
| Rebuild image and start container with logging     | `npm run docker:rebuild`                                            | `npm run docker:up -- --build`                                           |
| Rebuild image and start in detached mode           | `npm run docker:rebuild -- -d`                                      | `npm run docker:up -- --build -d`                                        |
| Stop locally built container & remove resources    | `npm run docker:down`                                               | `docker compose -f docker-compose.local.yml down`                        |
| Remove locally built image                         | `npm run docker:image-rm`                                           | `docker image rm compoviz-local:latest`                                  |
| Prune local Docker build cache                     | `npm run docker:prune`                                              | `docker builder prune -f`                                                |
| All-in-one stop compose, remove image, prune cache | `npm run docker:clean`                                              | `npm run docker:down && npm run docker:image-rm && npm run docker:prune` |
